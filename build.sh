...
RESULT_DIR="$BUILD/afetch-$ANDROID_ABI-$BUILD_TYPE"
...
process_aidl ()
{
  ...
  if [[ ! -f "$JAVA_BUILD_DIR/.aidl.stamp" ]] \
    ...
    touch "$JAVA_BUILD_DIR/.aidl.stamp"
  else
    echo "[*] No aidl changes, skip aidl"
  fi
}
...
package_all ()
{
  ...
  echo "[*] Done build afetch-$ANDROID_ABI-$BUILD_TYPE"
}
